...
model.S.set_allow_extrapolation(True)
model.B.set_allow_extrapolation(True)
model.theta.set_allow_extrapolation(True)
model.p.set_allow_extrapolation(True)
...
ax2.grid()
plt.tight_layout()
plt.savefig(out_dir + 'profile_plot_depth.pdf')
plt.close(fig)
